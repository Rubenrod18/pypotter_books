...
class TestUpdateCountry(_CountryBaseIntegrationTest):
    def test_is_country_updated_country_exist_previously_returns_country(
        ...
        with self.app.app_context():
            country_id = self.get_rand_country().id
            exclude = ['currency']
            data = ignore_keys(
            ...
            self.assertEqual(country_id, json_data.get('id'))
            self.assertEqual(
                data['currency_id'], json_data['currency'].get('id')
            )
            self.assertEqual(data['name'], json_data.get('name'))
            self.assertEqual(
                data['alpha_2_code'], json_data.get('alpha_2_code')
            )
            self.assertEqual(
                data['alpha_3_code'], json_data.get('alpha_3_code')
            )
